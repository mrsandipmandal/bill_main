<?php
ini_set("display_errors", "1");
error_reporting(E_ALL);
include "membersonly.inc.php";

$Members  = new isLogged(1);
$grpnm = "";
if (isset($_REQUEST['pnm'])) {
	$page_title = base64_decode($_REQUEST['pnm']);
} else {
	$page_title = "";
}
$user_current_level = $Members->User_Details->userlevel;
$user_current_bcd = $Members->User_Details->bcd;
$user_currently_loged = $Members->User_Details->username;

include "header.php";
date_default_timezone_set('Asia/Kolkata');
$sl=$_REQUEST['sl'];
$prd  = new Init_Table();
$prd->set_table("main_drcr","sl");
$prd->find($sl);

if($prd->cldgr=='-1')
	{$ldgr=$prd->dldgr;
	 $drcr="1";
	}
	else
	{$ldgr=$prd->dldgr;;
	$drcr="-1";
	}
	$dt=date('d-m-Y', strtotime($prd->dt));
?>
<!-- Left side column. contains the logo and sidebar -->
<aside class="main-sidebar">
	<!-- sidebar: style can be found in sidebar.less -->
	<section class="sidebar">
		<!-- Sidebar user panel -->
		<div class="user-panel">
			<div class="pull-left image">
				<img src="dist/img/user2-160x160.jpg" class="img-circle" alt="User Image">
			</div>
			<div class="pull-left info">
				<p><?php echo $Members->User_Details->name; ?></p>
				<a href="#"><i class="fa fa-circle text-success"></i> Online</a>
			</div>
		</div>
		<!-- sidebar menu: : style can be found in sidebar.less -->
		<?php echo include "left_bar.php"; ?>
	</section>
	<!-- /.sidebar -->
</aside>

<!-- Content Wrapper. Contains page content -->
<div class="content-wrapper">
	<!-- Content Header (Page header) -->
	<section class="content-header">
		<h1>
			<?php echo $page_title; ?>
			<small>Control panel</small>
		</h1>
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
			<li class="active"><?php echo $page_title; ?></li>
		</ol>
	</section>

	<!-- Main content -->
	<section class="content">
		<div class="row">

			<div class="col-md-12">
				<!-- general form elements -->
				<div class="box box-primary">
					<div class="box-header with-border">
						<h3 class="box-title"><?php echo $page_title; ?></h3>
					</div>
					<!-- /.box-header -->
					<!-- form start -->
					<div class="box-body">

						<form method="POST" action="openings.php" enctype="multipart/form-data">
						<input type="hidden" value="<?php echo $sl;?>"  id="sl" name="sl" />
									
						
							<div class="row">
							<div class="col-md-4">
									<div class="form-group">
										<label>BRANCH  : </label>
										<select id="bcd" name="bcd" class="form-control ch_req" required  onchange="get_vendor_op();">		
															
										<?php
										if($user_current_level<0)
										{
										?>
										
										<?php
										}
										else
										{
											
											$fld2['sl']=$user_current_bcd;
											$op2['sl']="IN, and";	
											
										}
											
											$fld2['brnch']='';
											$op2['brnch']="!=, ";

											$plist  = new Init_Table();
											$plist->set_table("main_brnch", "sl");
											 $group2=$plist->search_custom($fld2,$op2,'',array('brnch' => 'ASC'));

											foreach($group2 as $key2=>$ptp)
											{
												
											?>
											<option value="<?php echo $ptp['sl']; ?>"<?php if($ptp['sl']==$prd->bcd){ echo 'selected';} ?>><?php echo $ptp['brnch']; ?></option>
											<?php
											
											}
											?>
										</select>
									</div>
								</div>	
								<div class="col-md-4">
									<div class="form-group">

										<label>LEDGER HEAD : </label>

										<select id="ldgr" name="ldgr" class="form-control ch_req" required   onchange="get_vendor_op();" >
										<option value="">---Select---</option>										
										<?php
										$fld_ldgr['sl']='-1';
										$op_ldgr['sl']="!=, ";

										$plist  = new Init_Table();
										$plist->set_table("main_ledg", "sl");
										$group2=$plist->search_custom($fld_ldgr,$op_ldgr,'',array('nm' => 'ASC'));

										foreach($group2 as $key2=>$ptp)
										{
										

										?>
										<option value="<?php echo $ptp['sl']; ?>"<?php if($ptp['sl']==$ldgr){ echo 'selected';} ?>><?php echo $ptp['nm']; ?></option>
										<?php

										}
										?>
										</select>
									</div>
								</div>
								<?php
								$disp="";
								$req="required";
								if($ldgr!='21'){$disp="display: none;";$req="";}
								?>
								<div class="col-md-4" id="vendor_div" style="<?php echo $disp;?>" >
									<div class="form-group">
										<label>VENDOR : </label>
										<div id="get_vendor">
										<select id="vendor" name="vendor" class="form-control ch_req" <?php echo $req;?>  >
											<option value="">---Select---</option>
											<?php
											$fld2_v['bcd']=$prd->bcd;
											$op2_v['bcd']="=, ";

											$plist  = new Init_Table();
											$plist->set_table("main_vendor", "sl");
											$group2=$plist->search_custom($fld2_v,$op2_v,'',array('sl' => 'ASC'));

											foreach($group2 as $key2=>$ptp)
											{
											

											?>
											<option value="<?php echo $ptp['sl']; ?>"<?php if($ptp['sl']==$prd->vendor){ echo 'selected';} ?>><?php echo $ptp['nm']; ?> ( <?php echo $ptp['pan'];?> )</option>
											<?php

											}
											?>
										</select>

									</div>
									</div>
								</div>			
								<!-- end col-3 -->
								<div class="col-md-4">
									<div class="form-group">

										<label>DATE  : </label>

										<input type="text" value="<?php echo $dt;?>"  id="dt" name="dt" required class="form-control datepicker" />
									</div>
								</div>		
								<div class="col-md-4">
									<div class="form-group">

										<label>AMMOUNT ( Rs. ) : </label>

										<input type="number" value="<?php echo $prd->amm;?>"  id="amm" step="any" name="amm" required class="form-control brd" />
									</div>
								</div>	
								<div class="col-md-4">
									<div class="form-group">

										<label>DR/CR : </label>

										<select id="drcr" name="drcr" class="form-control " >
											<option value="1"<?php if($drcr=='1'){ echo 'selected';} ?>>DR</option>
											<option value="-1"<?php if($drcr=='-1'){ echo 'selected';} ?>>CR</option>
										
										</select>
									</div>
								</div>	
								<div class="col-md-4">
									<div class="form-group">

										<label>NARRATION : </label>

										<input type="text" value="<?php echo $prd->nrtn;?>"  id="nrtn" name="nrtn"  class="form-control brd" />
									</div>
								</div>	
							

							
							</div>
					


							<div class="row">
								<input type="hidden" name="typ" value="11" class="form-control" />
								<input type="hidden" name="table_name" value="main_drcr" class="form-control" />
								<input type="hidden" name="page_name" value="opening.php" class="form-control" />
								<!-- begin col-6 -->
								<div class="col-md-12">
									<div class="form-group" id="fl_fld">

									</div>
									<!-- end col-6 -->
									<div class="form-group pull-left">
										<div class="col-md-12">
											<button type="submit" class="btn btn-sm btn-success">Submit</button>
										</div>
									</div>


								</div>

							</div>
						</form>


											



						<!-- /.box body -->
					</div>
					

					
					<!-- /.box body -->
				</div>
				<div class="box box-success">
					
					<div class="box-body">
					<div class="row" id="show">						



					</div>
					</div>


					</div>



			</div>
	</section>
	<!-- /.content -->
</div>
<!-- /.content-wrapper -->



<!-- Add the sidebar's background. This div must be placed
       immediately after the control sidebar -->
<div class="control-sidebar-bg"></div>
</div>
<!-- ./wrapper -->

<?php
include "footer.php";
?>
<script>

window.onload = function() {
    if (window.jQuery) {  
        // jQuery is loaded  
     
    } 
}




$('#bcd').chosen({no_results_text: "Oops, nothing found!",allow_single_deselect: true,});
$('#ldgr').chosen({no_results_text: "Oops, nothing found!",allow_single_deselect: true,});
$('#vendor').chosen({no_results_text: "Oops, nothing found!",allow_single_deselect: true,});
</script>

</body>


</html>